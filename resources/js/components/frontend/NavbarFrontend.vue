        ...
        <nav class="bg-[var(--primary-blue)] shadow-md dark:bg-[var(--dark-background)]">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex h-16 items-center justify-between">
                    ...
                    <!-- Navbar Icons Mobile -->
                    <!-- Navbar Icons Mobile -->
                    <!-- <div class="flex w-full items-center justify-center space-x-6 md:hidden">
                        <Link prefetch :href="about()" class="text-[var(--dark-gold)] mobile-nav-item" aria-label="À propos" title="À propos">
                            <FontAwesomeIcon icon="info-circle" class="text-xl dark:text-[var(--dark-gold)]" />
                              <span>À propos</span>
                        </Link>

                        <Link prefetch :href="contact()" class="text-[var(--dark-gold)] mobile-nav-item" aria-label="Contact" title="Contact">
                            <FontAwesomeIcon
                                icon="envelope"
                                class="h-5 w-5 text-xl transition-transform duration-200 group-hover:scale-110 md:text-[var(--dark-gold)] dark:text-[var(--dark-gold)]"
                            />
                              <span>Contact</span>
                        </Link>
                        <Link
                            prefetch
                            :href="blogFrontend.index().url"
                            class="text-[var(--dark-gold)] mobile-nav-item"
                            aria-label="Acceder à notre blogs"
                            title="Blogs"
                        >
                        ...
                        <template v-if="isAuthenticated">
                            <Link prefetch href="/dashboard" class="text-[var(--dark-gold)] mobile-nav-item" aria-label="Dashboard" title="Dashboard">
                                <FontAwesomeIcon icon="tachometer-alt" class="text-xs dark:text-[var(--dark-gold)]" />
                                <span>Dashboard</span>
                            </Link>

                            <Link prefetch href="/logout" method="post" class="text-[var(--dark-gold)] mobile-nav-item" aria-label="Déconnexion" title="Déconnexion">
                                <FontAwesomeIcon icon="sign-out-alt" class="text-xs dark:text-[var(--dark-gold)]" />
                                <span>Log out</span>
                            </Link>
                        </template>

                        <template v-else>
                            <Link prefetch href="/login" class="text-[var(--dark-gold)] mobile-nav-item" aria-label="Se connecter" title="Se connecter">
                                <FontAwesomeIcon icon="right-to-bracket" class="text-xl md:text-[var(--dark-gold)] dark:text-[var(--dark-gold)]" />
                                <span>Login</span>
                            </Link>

                            <Link prefetch href="/register" class="text-[var(--dark-gold)] mobile-nav-item" aria-label="S’inscrire" title="S’inscrire">
                                <FontAwesomeIcon icon="user-plus" class="text-xl md:text-[var(--dark-gold)] dark:text-[var(--dark-gold)]" />
                                <span>register</span>
                            </Link>
                        </template>
