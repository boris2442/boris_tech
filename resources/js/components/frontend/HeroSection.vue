...
import { Code, Globe, Headphones, RotateCcw, Shield, Truck, UserCheck, Wrench } from 'lucide-vue-next';
...
Bienvenue à Boris Tech, votre boutique de référence pour tous les équipements informatiques et
...
Chez Boris Tech, vos commandes se passent directement sur WhatsApp, ce qui rend l’expérience simple,
...
Boris Tech propose régulièrement des offres exceptionnelles, ventes flash et remises limitées sur
...
Avec Boris Tech, vous achetez en toute confiance. Tous nos produits sont authentiques, garantis, et
...
const features = [
    {
        icon: Truck,
        text: 'Livraison rapide et sécurisée dès 3h selon la zone',
    },
    {
        icon: Shield,
        text: 'Produits électroniques authentiques, certifiés et garantis',
    },
    {
        icon: Headphones,
        text: 'Service après-vente Boris Tech – assistance et support agréés',
    },
    {
        icon: RotateCcw,
        text: 'Retour simple et rapide sous 7 jours',
    },
    {
        icon: Code,
    ...
    {
        icon: Globe,
        text: 'Création de sites web professionnels et e-commerce',
    },
    {
        icon: Wrench,
        text: 'Maintenance, mise à jour et optimisation technique',
    },
    {
        icon: UserCheck,
        text: 'Accompagnement et conseils personnalisés pour vos projets',
    },
];
</script>
...
<template>
    ...
    >
        ...
        <div class="relative z-10 flex max-w-4xl -translate-x-2 transform flex-col gap-6 md:-translate-x-0">
            ...
            <div class="flex">
                <p
                    :class="[
                        'text-sm leading-relaxed text-[var(--secondary-white)] sm:text-base md:text-base dark:text-[var(--dark-grey)]',
    ...
    <section class="mx-auto w-full bg-gray-100 pt-8 pb-2 dark:bg-[var(--dark-map2)]">
        <div class="mx-auto grid max-w-6xl grid-cols-2 gap-4 p-2 md:grid-cols-4">
            <div
            ...
            >
                <component :is="item.icon" class="h-6 w-6 text-yellow-600 group-hover:text-[#FFE600] md:h-8 md:w-8" />
